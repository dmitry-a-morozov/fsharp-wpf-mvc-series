...
type CalculatorController() = 
    ...
    override this.InitModel model = 
        ...
        model.SelectedOperation <-  Models.Operations.Add
        model.X <- 0
        model.Y <- 0
        model.Result <- 0
    ...
    member this.YChanged text model = 
        if text <> "0"
        then 
            model.AvailableOperations <- Models.Operations.Values
        else 
            model.AvailableOperations <- Models.Operations.Values |> Array.filter(fun op -> op <> Models.Operations.Divide)

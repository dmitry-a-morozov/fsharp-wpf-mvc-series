...
type StockPricesChartController() = 
    ...
    override this.Dispatcher = fun() -> 
        Async <| fun model ->
            async {
                let childModel, view, contoller = StockInfoModel(), StockPickerView(), StockPickerController()
                let! result = Mvc(childModel, view, contoller).StartWindow()  
                if result 
                then
                    model.StocksInfo.Add childModel
                    model.SelectedStock <- childModel
            }
